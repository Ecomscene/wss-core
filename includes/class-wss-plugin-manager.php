...
class WSS_Plugin_Manager {
	...
	public function sync() {
		...
		foreach ( (array) $resp['plugins'] as $p ) {
			...
			// For reinstalls of an active plugin, Plugin_Upgrader::install() with
			// overwrite_package=true can silently fail to replace files (returns
			// true but leaves the old version in place). Workaround: deactivate
			// → reinstall → reactivate.
			$was_active = ! $is_first_install && is_plugin_active( $basename );
			if ( $was_active ) {
			...
			if ( ! $installed ) {
				...
				// If we deactivated for a reinstall that then failed, restore the prior active state.
				if ( $was_active ) {
					ob_start(); activate_plugin( $basename, '', false, false ); ob_end_clean();
				}
				continue;
			}
			...
			// First-time install: honour the desired state.
			...
			// Reinstall: restore prior active state, regardless of desired_state
			// (the user's imperative buttons own activation after first install).
			if ( ! $is_first_install && $was_active ) {
				ob_start();
				$r = activate_plugin( $basename, '', false, false );
				ob_end_clean();
				if ( is_wp_error( $r ) ) {
					$results[ $basename ] = 'reinstalled+activate_failed: ' . $r->get_error_message();
				} else {
					$results[ $basename ] = 'reinstalled+reactivated';
				}
			}
		}
